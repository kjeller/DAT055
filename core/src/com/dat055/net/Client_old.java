 ...
 */
public class Client_old extends Thread {
    ...
    int t = 0;
    public Client_old(InetAddress addr, int port) {
        this.addr = addr;
        this.port = port;
        try {
    ...
    public void run() {
        while(!interrupted()) {
            System.out.println("[Line: 27] Client_old running."+t++);
            try {
                Thread.sleep(20);
    ...
    private void sendPacket() {
        if(data != null) {
            ...
            try {
                System.out.printf("==> Client_old sent UDP packet to %s:%d \n", addr.getHostAddress(), port);
                ds.send(packet);
     ...
     */
    public void close() {
        System.out.println("!!Client_old thread and ds will be closed.!!");
        ds.close();
        try {
            if(ps != null)
                ps.close();
